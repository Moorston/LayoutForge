...
function htmlToJsx(html: string): string {
  return html
    ...
    .replace(/<(img|input|br|hr|meta|link|area|base|col|embed|param|source|track|wbr)(\s[^>]*)?>(?!<\/)/gi,
...
function exportAsHtml(html: string, css: string, pageName = 'index'): ExportResult {
...
function exportAsReact(html: string, css: string, componentName = 'MyComponent'): ExportResult {
...
function exportAsVue(html: string, css: string, componentName = 'MyComponent'): ExportResult {
  const name = pascalCase(componentName) || 'MyComponent';
...
function exportAsNextjs(html: string, css: string, pageName = 'page'): ExportResult {
  const jsx = htmlToJsx(html);
...
export function exportCode(
  ...
  switch (format) {
    case 'html':   return exportAsHtml(html, css, name);
    case 'react':  return exportAsReact(html, css, name);
    case 'vue':    return exportAsVue(html, css, name);
    case 'nextjs': return exportAsNextjs(html, css, name);
    default:       return exportAsHtml(html, css, name);
  }
}
...
export const FORMAT_META: Record<ExportFormat, { label: string; icon: string; desc: string; color: string }> = {
  html:   { label: 'HTML',    icon: '🌐', desc: 'Single-file HTML with Tailwind CDN', color: 'text-orange-600 bg-orange-50 border-orange-200' },
  react:  { label: 'React',   icon: '⚛️', desc: 'TypeScript functional component (.tsx)', color: 'text-blue-600 bg-blue-50 border-blue-200' },
  vue:    { label: 'Vue 3',   icon: '💚', desc: 'Single File Component with <script setup>', color: 'text-emerald-600 bg-emerald-50 border-emerald-200' },
  nextjs: { label: 'Next.js', icon: '▲',  desc: 'App Router page component + CSS module', color: 'text-slate-900 bg-slate-100 border-slate-300' },
};
